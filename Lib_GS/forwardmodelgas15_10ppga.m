...
sigma_k = 10^(sigma_k_log);     % MUST be scalar
qn_row  = reshape(qn, 1, []);   % force 1xN row
assert(~isempty(qn) && isvector(qn), 'qn must be a non-empty vector.');
assert(~isempty(sigma_k_log) && isscalar(sigma_k_log) && isfinite(sigma_k_log), ...
    'sigma_k_log must be a finite scalar.');

sigma_row = sigma_k .* (qn_row.^(1/3));    % 1xN
sigma_ome = 1 ./ sigma_row;               % 1xN
